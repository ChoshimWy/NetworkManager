...
extension DiskStorage: StorageAware {
  ...
  public func removeExpiredObjects() throws {
    ...
    for url in urlArray {
      ...
      if let expiryDate = resourceValues.contentModificationDate, expiryDate.timeIntervalSinceNow < 0 {
...
extension DiskStorage {
  ...
  func removeObjectIfExpired(forKey key: String) throws {
    let filePath = makeFilePath(for: key)
    let attributes = try fileManager.attributesOfItem(atPath: filePath)
    if let expiryDate = attributes[.modificationDate] as? Date, expiryDate.timeIntervalSinceNow < 0 {
      try fileManager.removeItem(atPath: filePath)
      onRemove?(filePath)
    }
  }
